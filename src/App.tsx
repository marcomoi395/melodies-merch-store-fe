...
import { discountedPrice, formatMoney, mergeOrderItems, type OrderItemInput } from "./storefront";
...
type Cart = { id: string; cartItems: Array<{ id: string; quantity: number; product: Product }> };
...
function Detail({ navigate, slug, user }: { navigate: (to: string) => void; slug: string; user: User | null }) {
  ...
  const add = async () => { if (!selected) return; if (!user) return navigate(`/login?next=${encodeURIComponent(`/products/${product.slug}`)}`); try { await api.post("/cart", { productId: product.id, productVariantId: selected.id, quantity: 1 }, { authenticated: true }); setNotice("Đã thêm vào giỏ hàng."); } catch (reason) { setError(message(reason)); } };
...
function CartPage({ navigate }: { navigate: (to: string) => void }) {
  const [cart, setCart] = useState<Cart | null>(null); const [error, setError] = useState(""); const [loading, setLoading] = useState(true);
  const load = () => { setLoading(true); setError(""); api.getCart<Cart>().then(setCart).catch((reason) => setError(message(reason))).finally(() => setLoading(false)); };
  useEffect(load, []);
  const change = async (id: string, quantity: number) => { try { setCart(await api.patchCart<Cart>(id, quantity)); } catch (reason) { setError(message(reason)); } };
  const remove = async (id: string) => { try { setCart(await api.removeCart<Cart>(id)); } catch (reason) { setError(message(reason)); } };
  if (loading) return <Empty>Đang tải giỏ hàng...</Empty>;
  if (!cart) return <section className="content-section"><Notice error>{error}</Notice><button className="button ghost" onClick={load}>THỬ LẠI</button></section>;
  return <section className="content-section"><h1>GIỎ HÀNG</h1>{error && <Notice error>{error}</Notice>}{cart.cartItems.length === 0 ? <Empty>Giỏ hàng đang trống.</Empty> : <><div className="cart-list">{cart.cartItems.map((item) => { const variant = item.product.variants[0]; return <article key={item.id} className="cart-item"><Media media={item.product.mediaGallery} name={item.product.name} /><div><strong>{item.product.name}</strong><p>{variant?.name}</p><p>{variant && formatMoney(discountedPrice(variant.originalPrice, variant.discountPercent))}</p></div><div className="quantity"><button aria-label="Giảm số lượng" onClick={() => change(item.id, item.quantity - 1)}>-</button><span>{item.quantity}</span><button aria-label="Tăng số lượng" onClick={() => change(item.id, item.quantity + 1)}>+</button><button className="text-action" onClick={() => remove(item.id)}>XÓA</button></div></article>; })}</div><button className="button" onClick={() => navigate("/checkout?cart=1")}>THANH TOÁN COD</button></>}</section>;
}
...
function Checkout({ navigate, search, user }: { navigate: (to: string) => void; search: string; user: User | null }) {
  ...
  useEffect(() => { if (params.get("cart") === "1") { api.getCart<Cart>().then((cart) => setItems(cart.cartItems.flatMap((item) => { const variant = item.product.variants[0]; return variant ? [{ productVariantId: variant.id, quantity: item.quantity }] : []; }))).catch((reason) => setError(message(reason))).finally(() => setLoading(false)); } else { const variant = params.get("variant"); setItems(variant ? [{ productVariantId: variant, quantity: Math.max(1, Number(params.get("quantity") ?? "1")) }] : []); setLoading(false); } }, [params]);
  ...
  const create = async () => { if (!preview) return getPreview(); if (!form.fullName.trim() || !/^\S+@\S+\.\S+$/.test(form.email) || !form.phone.trim()) return setError("Hãy nhập họ tên, email hợp lệ và số điện thoại."); try { const order = await api.post<Order>("/order", { ...form, note: form.note || undefined, items: mergeOrderItems(items), paymentMethod: "COD" }, { authenticated: Boolean(browserTokens.read()) }); sessionStorage.setItem("melodies.last-order", JSON.stringify(order)); navigate("/order/success"); } catch (reason) { setPreview(null); setError(`${message(reason)}. Vui lòng xem lại đơn hàng.`); } };
...
function App() {
  ...
  else if (pathname.startsWith("/products/")) page = <Detail navigate={navigate} slug={decodeURIComponent(pathname.slice(10))} user={user} />;
  ...
  else if (pathname === "/cart") page = user ? <CartPage navigate={navigate} /> : <Auth navigate={navigate} onLogin={setUser} pathname="/login" search="?next=%2Fcart" user={user} />;
